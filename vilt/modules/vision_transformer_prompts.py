...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, prompts=None, learnt_p=False, prompt_type='input'):
        ...
        if prompts is None or prompt_type == 'input':
            # origin attention
            qkv = (
                self.qkv(x) #projection to keys, values and queries
                .reshape(B, N, 3, self.num_heads, C // self.num_heads) #C // self.num_heads=dimensionality of each head
                .permute(2, 0, 3, 1, 4) #(3, B, self.num_heads, N, C // self.num_heads)
            )
            """
            Dimension 0: Represents the components (queries, keys, values)
            Dimension 1 = B =batch_size
            Dimension 2 = number of attention heads
            Dimension 3 = N = number of tokens
            Dimension 4 = dimensionality of each head
            """
            q, k, v = (
                qkv[0],
                qkv[1],
            ...
            mask = mask[:,start_pos:] #keep only the last N elements of the mask tensor
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale #dot product between queries and keys then scaled
        #(1, B, self.num_heads, N, N)
        if mask is not None: #Checks if a mask is provided
            mask = mask.bool() # Converts the mask tensor to boolean type.
            attn = attn.masked_fill(~mask[:, None, None, :], float("-inf")) #Masks the attention scores where the mask is False by replacing them with -inf
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        #attn has shape (B, num_heads, N, N)
        #v has shape (B, num_heads, N, head_dim)
        #attn @ v has shape (B, num_heads, N, head_dim)
        # (attn @ v).transpose(1, 2) = (B, N, num_heads, head_dim)
        # x = (B, N, C=num_heads * head_dim)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        num_patches = (img_size[1] // patch_size[1]) * (img_size[0] // patch_size[0]) #H/P * W/P
    ...
    def forward(self, x):
        ...
        x = self.proj(x) #(B, embed_dim=768, Hout=14, Wout=14)  
        return x
...
#             trunc_normal_(self.mask_token, std=0.02)
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        """
        Patch Embedding and Mask Calculation:
        """
        _, _, ph, pw = self.patch_embed.proj.weight.shape

        x = self.patch_embed(_x) #patch embedding of the input  
        x_mask = (_x.sum(dim=1) != 0).float()[:, None, :, :] #Calculates a mask based on the sum of values along the channel dimension of _x
        x_mask = F.interpolate(x_mask, size=(x.shape[2], x.shape[3])).long() # resizes the mask tensor to match the spatial dimensions of tensor x using a bilinear interpolation
        #Computes sums of non-zero elements along the height and width dimensions to determine effective image sizes 
        ...
        """
        B, C, H, W = x.shape
        #Computes positional embeddings based on the spatial positions of the patches.
        spatial_pos = (
            self.pos_embed[:, 1:, :] #pos embedding for the tokens
            .transpose(1, 2)
            .view(1, C, self.patch_dim, self.patch_dim)
        )
        #Interpolates spatial position embeddings to match the spatial dimensions of the patch embeddings
        pos_embed = torch.cat(
            [
                F.pad(
                    F.interpolate(
        ...
        #Flattens and transposes the position embeddings for concatenation with the patch embeddings.
