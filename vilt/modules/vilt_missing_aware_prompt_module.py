...
class ViLTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        self.text_embeddings = BertEmbeddings(bert_config) #Bert embedding
...
# 
        ...
        complete prompt
        """
        complete_prompt = torch.zeros(prompt_num, prompt_length, embed_dim) #(6, 16, embed_dim=hidden_size)
        complete_prompt[:,0:1,:].fill_(1) # sets the first token of each prompt to 1
        if self.learnt_p and self.prompt_type == 'attention': #prompt_type='input'
            complete_prompt[:,prompt_length//2:prompt_length//2+1,:].fill_(1)
        self.complete_prompt = nn.Parameter(complete_prompt) #conversion into learnable parameters

        """
        missing text prompt
        """
        missing_text_prompt = torch.zeros(prompt_num, prompt_length, embed_dim)
        missing_text_prompt[:,2:3,:].fill_(1) #set the third token of each prompt to 1         
...
#                     image_embeds[idx,1:].fill_(1)
        ...
        #adding token type embeddings to both text and image embeddings based on their respective token masks and token type indices.
        ...
        prompts = None
        for idx in range(len(img)): #selct the prompt
        ...
        if self.learnt_p: #true
            ...
            elif self.prompt_type=='input': #input
        ...
        for i, blk in enumerate(self.transformer.blocks):
            if i in self.prompt_layers: #add the prompt only on the first 6 layers
        ...
        if self.prompt_type == 'input':
            cls_feats = self.pooler(x[:,total_prompt_len:total_prompt_len+1]) #first token after prompts   
